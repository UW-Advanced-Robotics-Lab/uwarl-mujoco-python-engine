...
class ControlCommand(object):
    ...
    def __init__(self,mj_data):
        ...
        # Summit
        self.sub_vel_base_summ = rospy.Subscriber("uwarl_a/robotnik_base_control/cmd_vel", Twist, self.vel_summ_base_callback)
        # Fetch
        self.sub_vel_base_fetch = rospy.Subscriber("fetch/fetch_base_control/cmd_vel", Twist, self.vel_fetch_base_callback)
    ...
    def vel_summ_base_callback(self, msg):
        ...
        self.vel_summ_base = [msg.linear.x, msg.linear.y, msg.angular.z]
    
    def vel_fetch_base_callback(self, msg):
        # Store data in array to be used by PID control
        # New velocity commands are received at approximately 50 Hz, PID loop runs at 200 Hz
        self.vel_fetch_base = [msg.linear.x, msg.linear.y, msg.angular.z]


    # PID loop to control x velocity of holonomic-robot in map frame
    def velx_holo_PID(self, Kp, Ki, Kd, CP, base_name):

        # Convert reference velocities into mapframe
        SP = self.vel_summ_base[0]*cos(self.mj_data_control.joint(base_name+'/orie/z').qpos[0])+self.vel_summ_base[1]*-sin(self.mj_data_control.joint(base_name+'/orie/z').qpos[0])
    ...
    def velx_non_holo_PID(self, Kp, Ki, Kd, CP, base_name):
    ...
    def vely_holo_PID(self, Kp, Ki, Kd, CP, base_name):

        # Convert reference velocities into mapframe
        SP = self.vel_summ_base[0]*sin(self.mj_data_control.joint(base_name+'/orie/z').qpos[0])+self.vel_summ_base[1]*cos(self.mj_data_control.joint(base_name+'/orie/z').qpos[0])
    ...
    def veltheta_holo_PID(self, Kp, Ki, Kd, CP,base_name):

        SP = self.vel_summ_base[2]
